...
describe('PATCH /subscriptions/:subscriptionId/cancel', () => {
	it('should cancel an active subscription and return 204', async () => {
	...
	it('should return 404 if subscription does not exist', async () => {
		const response = await app.inject({
			method: 'PATCH',
			url: `/subscriptions/${faker.string.uuid()}/cancel`,
		})
